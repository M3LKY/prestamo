...
app.post("/login", async (req, res) => {
  try {
    ...
    const admin = await pool.query("SELECT * FROM admin WHERE password = $1", [
      password,
    ]);
...
app.post("/addAdmin", async (req, res) => {
  try {
    ...
    const newAdmin = await pool.query(
      `INSERT INTO admin (firstname, lastname, contactnumber, address, email, password, username, level) VALUES ('${firstname}', '${lastname}', '${contactNumber}', '${address}', '${email}', '${password}', '${username}', '${level}') RETURNING *`
    );
...
console.log("aqui:  "+getClient.rows[0])
...
app.get("/allLoans", auth, async (req, res) => {
  try {
    const getLoans = await pool.query(
      `SELECT c.firstname, c.lastname, l.id, l.client_id, l.type, l.interes, l.monto, l.dia, l.rdate, l.cuota, l.pagado FROM newloans AS l INNER JOIN clients AS c ON l.client_id = c.id WHERE c.id = l.client_id`
    );

    res.json(getLoans.rows);
    // console.log(getLoans.rows);
  } catch (error) {
    console.log(error.message);
  }
});
...
app.get("/loanns/:id", auth, async (req, res) => {
  try {
    const id = req.params.id;

    const getLoan = await pool.query(
      `SELECT c.firstname, c.lastname, l.id, l.client_id, l.type, l.interes, l.monto, l.dia, l.rdate, l.cuota, l.pagado FROM newloans AS l INNER JOIN clients AS c ON l.client_id = c.id WHERE l.id = ${id}`
    );

    if (getLoan.rows.length === 0) {
      return res.status(404).json({ message: "Loan not found" });
    }

    res.json(getLoan.rows); // Assuming there should be only one loan with a given ID
    // console.log(getLoan.rows);
  } catch (error) {
    console.log(error.message);
    res.status(500).json({ message: "Internal Server Error" });
  }
});
app.post("/updatePagado/:id", auth, async (req, res) => {
  try {
    const id = req.params.id;
    let newPagadoValue = req.body.newPagado;
    // console.log(newPagadoValue);
    // Check if newPagadoValue is NaN or an empty string
    if (isNaN(newPagadoValue) || newPagadoValue === "") {
      // Handle the situation where newPagadoValue is NaN or an empty string
      res.status(400).json({ message: "Invalid value for pagado" });
      return;
    }
    if (newPagadoValue === null) {
      newPagadoValue = 0;
    }

    // Convert newPagadoValue to string explicitly
    newPagadoValue = String(newPagadoValue);

    // Perform an update or insert based on the value
    ...
    await pool.query(updatePagadoQuery, [newPagadoValue, id]);

    res.json({ message: "Pagado updated successfully" });
  } catch (error) {
    console.log(error.message);
    res.status(500).json({ message: "Internal Server Error" });
  }
...
app.get("/loans/:id", auth, async (req, res) => {
  try {
    ...
    const getClient = await pool.query(
      `SELECT c.firstname, c.lastname, l.id, l.client_id, l.type, l.interes, l.monto, l.dia, l.rdate, l.cuota, l.pagado FROM newloans AS l INNER JOIN clients AS c ON l.client_id = c.id WHERE c.id = ${id}`
    );

    res.json(getClient.rows);
    // console.log("loans: " + getClient.rows);
  } catch (error) {
    console.log(error.message);
  }
});
...
app.get("/dates", auth, async (req, res) => {
  try {
    ...
    const getLoan = await pool.query(`SELECT rdate FROM newloans`);
...
app.post("/loans/:id", auth, async (req, res) => {
  try {
    ...
    const { type, interes, monto, dia, date, cuota, pagado } = req.body;
    console.log(req.body);

    const newLoan = await pool.query(
      "INSERT INTO newloans(client_id, type, interes, monto, dia, rdate, cuota, pagado) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *",
      [id, type, interes, monto, dia, date, cuota, pagado]
    );
...
app.patch("/loans/:id", auth, async (req, res) => {
  try {
    ...
    const updateLoan = await pool.query(
      `UPDATE newloans 
       SET type = $1, interes = $2, monto = $3, dia = $4, rdate = $5, cuota = $6 
       WHERE id = $7 RETURNING *`,
      [type, interes, monto, dia, date1, cuota, id]
    );
...
app.delete("/loans/:id", auth, async (req, res) => {
  try {
    ...
    const deleteLoan = await pool.query(
      `DELETE FROM newloans WHERE id = ${id} RETURNING * `
    );
...
app.post("/payments/:id", auth, async (req, res) => {
  try {
    ...
    const addPayment = await pool.query(
      `INSERT INTO PAYMENTS (amount, collection_date, collected_by, new_balance, method, client_id, loan_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [
        amount,
        /^\d{4}-\d{2}-\d{2}$/.test(collection_date)
